...
function DrawTexRect(x1,y1,x2,y2,s1,t1,s2,t2)
	gl_TexRect(x1-offsetx,y1-offsety,x2-offsetx,y2-offsety,s1,t1,s2,t2)
end
...
function DrawComponent(number)
	...
	local x1,y1,x2,y2
	if number <9 then
		gl_Texture(':n:LuaUI/Images/res_energy.png')	
		...
		gl_Texture(':n:LuaUI/Images/res_metal.png')	

		x1= vsx +components[number].left -pngx -465  --bottom left of placing
		y1= vsy -components[number].top -math.abs(components[number].ty1-components[number].ty2) 
		x2= x1 +(components[number].tx2-components[number].tx1) 	--top right of placing
		y2= y1 +(components[number].ty1-components[number].ty2 )
	end
	local s1=components[number].tx1 /pngx--top left bounding
	local t1=(465-components[number].ty2) /pngy
	local s2=components[number].tx2/pngx--bottom right bounding
	local t2=(465-components[number].ty1) /pngy
--	gl.TexRect(x1-500,y1-500,x2-500,y2-500,s1,t1,s2,t2)

--	Spring.Echo( x1..' '..y1..' '..x2..' '..y2..' '..s1..' '..t1..' '..s2..' '..t2)
...
--	Spring.Echo(pct)
	if pct >0.15 then
		...
		gl_Texture(false)
			...
			t1=(465-components[number].ty2) /pngy
			s2=(components[number].tx2) /pngx--bottom right bounding
			t2=(465-components[number].ty1) /pngy
...
--	Spring.Echo(pct)
		...
		gl_Color(1,1,1,components[number].alpha)
		gl_Texture(':n:LuaUI/Images/res_metal.png')	
		local x1= vsx +components[number].left -pngx -465  --bottom left of placing
		local y1= vsy -components[number].top  -math.abs(components[number].ty1-components[number].ty2)  
		local x2= x1 +drawwidth+1	--top right of placing
...
function DrawMShare(ms)
	...
	x1= vsx +components[number].left -pngx -465 +pos --bottom left of placing
	y1= vsy -components[number].top -math.abs(components[number].ty1-components[number].ty2) 
	x2= x1 +(components[number].tx2-components[number].tx1)	--top right of placing
	y2= y1 +(components[number].ty1-components[number].ty2 )
...
function widget:MousePress(x, y, button)
	local x1,y1,x2,y2
	local number=1
	x1= vsx +components[number].left -pngx -offsetx --bottom left of placing
	y1= vsy -components[number].top -math.abs(components[number].ty1-components[number].ty2)   -offsety
	x2= x1 +(components[number].tx2-components[number].tx1)	 --top right of placing
	y2= y1 +(components[number].ty1-components[number].ty2 ) 
	...
	local number=10
	x1= vsx +components[number].left -pngx  -465 -offsetx --bottom left of placing
	y1= vsy -components[number].top -math.abs(components[number].ty1-components[number].ty2) -offsety
	x2= x1 +(components[number].tx2-components[number].tx1)	--top right of placing
	y2= y1 +(components[number].ty1-components[number].ty2 )
...
function widget:DrawScreen()
	...
	if Spring.GetGameFrame() > 1 then
		...
		if curMlevel<10000 then
			DrawText(math_floor(curMlevel), vsx-930+259, vsy-49,10,'cn')
		else
			DrawText(math_floor(curMlevel/1000)..'K', vsx-930+259, vsy-49,10,"cn")
		end

		if curMstore<10000 then
			DrawText(math_floor(curMstore), vsx-930+434, vsy-49,10,'cn')
		else
			DrawText(math_floor(curMstore/1000)..'K', vsx-930+434, vsy-49,10,"cn")
		end

		
		gl_Color(0,0.6,0,1)
		if curMinc<1000 then
			DrawText('+'..math_floor(curMinc)..'.'..math_floor((curMinc-math_floor(curMinc))*10), vsx-930+56, vsy-23,10,'cn')
		else
			DrawText('+'..math_floor(curMinc/1000)..'.'..math_floor((curMinc/1000-math_floor(curMinc/1000))*10) ..'K', vsx-930+56, vsy-23,10,"cn")
		end


		gl_Color(0.7,0,0,1)
		if curMpull<1000 then
			DrawText('-'..math_floor(curMpull)..'.'..math_floor((curMpull-math_floor(curMpull))*10), vsx-930+56, vsy-39,10,'cn')
		else
			DrawText('-'..math_floor(curMpull/1000) ..'.'.. math_floor((curMpull/1000-math_floor(curMpull/1000))*10)..'K', vsx-930+56, vsy-39,10,"cn")
		end
