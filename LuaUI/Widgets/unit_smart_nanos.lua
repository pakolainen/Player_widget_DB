...
function widget:GetInfo()
  return {
    name      = "SmartNanos",
    desc      = "Enables auto reclaim & repair for idle turrets 1.5",
...
function widget:Initialize()
  ...
  if Spring.GetSpectatingState() or Spring.IsReplay() then
    Spring.Echo("SmartNanos widget disabled for spectators")
...
function widget:UnitFinished(unitID, unitDefID, unitTeam)
  ...
  if UnitDefs[unitDefID].isCommander then myTeamID = GetMyTeamID() end
...
function widget:Update(deltaTime)
  ...
  if (timeCounter > UPDATE) then
    ...
    if (GetMyTeamID() ~= myTeamID) then
      Spring.Echo("SmartNanos widget disabled for team change")
      widgetHandler:RemoveWidget()
      return false
    end
