...
function widget:GetInfo()
	return {
		name = "Redo Team Colors",
		desc = "Pick bright evenly hue-spaced random new team colors",
		author = "zwzsg",
...
local function HSLfromRGB(Red,Green,Blue)
	local iR=Red
	local iG=Green
	local iB=Blue
	fR = iR/255
...
local function RGBfromHSL(Hue,Saturation,Lightness)
	local iH=Hue
	local iS=Saturation
	local iL=Lightness
	local fH = iH/255
...
local function SetNewTeamColors()
	...
	for p,_ in ipairs(TeamList) do
		...
		Spring.SetTeamColor(TeamList[p],Red/255,Green/255,Blue/255)
	end
end
