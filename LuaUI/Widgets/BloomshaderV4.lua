...
function widget:Initialize()
	...
	blurShaderH71 = glCreateShader({
		fragment = [[
			...
			void main(void) {
				const vec2 texCoors = vec2(gl_TexCoord[0]);
				vec4 samples[7];
				...
				samples[3] = (samples[0] + 6*samples[1] + 15*samples[2] + 20*samples[3] + 15*samples[4] + 6*samples[5] + samples[6]);
	...
	brightShader = glCreateShader({
		fragment = [[
			...
			void main(void) {
				const vec2 texCoors = vec2(gl_TexCoord[0]);
				const vec3 color = vec3(texture2D(texture0, texCoors));
				const float illum = dot(color, vec3(0.2990, 0.5870, 0.1140));
