...
local wayPtSelDist = 10
local selWayPtsTbl = {}

function widget:GetInfo()
	return {
		name      = "Waypoint dragger 3",
    version   = "3.4",
		desc      = "Enables Waypoint Dragging",
		author    = "Kloot, Pako",
		date      = "Aug. 8, 2007 [updated Aug. 25, 2011]",
		license   = "GNU GPL v2",
		layer     = 5,
		enabled   = true
	}
end
--TODO fix cursor switching-maybe make a command for this

function widget:Initialize()
  Spring.AssignMouseCursor("wp_edit", "wpedit", false)
end
...
local function MoveWayPoints(wpTbl, mx, my, finalize)
	...
	if (cursorWorldCoors ~= nil) then
        ...
        if cmdID < 0 then
          sprGiveOrderToUnit(cmdUnitID, CMD.INSERT, {cmdLink, cmdID, 0, cx, cy, cz, cmdRadius}, {""})
				elseif (cmdRadius > 0) then
					-- sprGiveOrderToUnit(cmdUnitID, CMD.INSERT, {cmdNum, cmdID, 0, cx, cy, cz, cmdRadius}, {"alt"})
          sprGiveOrderToUnit(cmdUnitID, CMD.INSERT, {cmdLink, cmdID, 0, cx, cy, cz, cmdRadius}, {""})
				else
...
function widget:MousePress(mx, my, mb)
	...
	--
	local idx, actCmdID, _, _      = sprGetActiveCommand()
	...
	if (cmdDesc and cmdDesc["type"] == 20 ) then
		--quit here if a build command
		return
	end
...
if mb == 3 then --minor bug: if customformation or some drawing command is started near this takes it
    for wpKey, wpData in pairs(selWayPtsTbl) do
      sprGiveOrderToUnit(wpData[7], CMD.REMOVE, {wpData[6].tag}, {""})
  end
end
...
local activeComm = false

function widget:DrawWorld()
	local mx, my, mb1, mb2, mb3 = sprGetMouseState()
	local _, _, _, shift = sprGetModKeyState()
	local wpTblTmp = {}
	if (not shift) then
		return
...
if not(mb2 or mb3) then
  nWp = GetWayPointsNearCursor(wpTblTmp, mx, my)
  if (nWp > 0 or next(selWayPtsTbl)) then
    Spring.SetMouseCursor("wp_edit")
  end
end
	...
	for _, wpData in pairs(selWayPtsTbl) do
		...
		if (cmd.id < 0) then
			local ret, _ = sprTestBuildOrder(-cmd.id, nx, ny, nz, wpData[6].params[#wpData[6].params])
			if (ret == 0) then
			...
			glRotate(wpData[6].params[#wpData[6].params] * 90.0, 0.0, 1.0, 0.0)
